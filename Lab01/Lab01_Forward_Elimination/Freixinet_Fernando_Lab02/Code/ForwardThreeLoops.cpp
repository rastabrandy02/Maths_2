...
void SmallPartialPivoting(float matrix[][SMALL + 1], int n, int loop);
...
int main(int argc, char* argv[])
{
	...
	cout << endl << "Expanding matrix..." << endl;
	SmallExpansion(smallMatrix, smallIndependent, smallExpanded, SMALL);
	cout << endl << "Starting triangulation..." << endl;
	...
	cout << endl << "Checking consistency of the triangulated matrix before backtracking..." << endl;
...
void SmallTriangulationThreeLoops(float matrix[][SMALL + 1], int n)
{
	...
	for (int i = 0; i < n - 1; i++)
	{
		SmallPartialPivoting(matrix, SMALL, i);
		...
		for(int j = i+1; j < n; j++)
		{
			...
			for (int k = i; k < n + 1; k++)
			{
				if (pivot > -0.1f &&  pivot < 0.1f)
				{
					cout << "A very small pivot was found in the iteration" << i << endl;
					matrix[j][k] = (pivot * matrix[j][k]) - (subPivot * matrix[i][k]);
				}
				else matrix[j][k] = matrix[j][k] - ((subPivot / pivot) * matrix[i][k]);
			}
		}
	}
}
...
void SmallPartialPivoting(float matrix[][SMALL +1], int n, int loop)
{
	...
	{
		if (abs(matrix[i - 1][loop]) < abs(matrix[i][loop]))
		{
			for (int j = 0; j <= n; j++)
			{
				aux = matrix[i][j];
